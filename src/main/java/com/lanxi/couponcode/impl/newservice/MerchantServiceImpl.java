...
//                    OutputStream os = null;
                    ...
                    //变更后的图片存贮与数据库中
                    boolean exists = picsService.testExist(merchant.getMerchantId());
                    byte[] bytes = new byte[is.available()];
                    is.read(bytes);
                    conn = ssf.openSession().getConnection();
                    if (!exists){
                        MerchantPics pics = new MerchantPics();
                        pics.setMerchantId(merchant.getMerchantId());
                        Blob         blob = conn.createBlob();
                        blob.setBytes(1, bytes);
                        pics.setOrganizingInstitutionBarCodePic(blob);
                        return picsService.addMerchantPics(pics);
                    }else{
                        Blob blob = picsService.getOrganizingInstitutionBarCodePic(merchant.getMerchantId());
                        if(blob==null){
                            blob = conn.createBlob();
                        }else{
                            blob.setBytes(1,new byte[(int)blob.length()]);
                        }
                        blob.setBytes(1,bytes);
                        result= picsService.updateOrganizingInstitutionBarCodePic(merchant.getMerchantId(),blob);
                        if(result){
                            merchant.setOrganizingInstitutionBarCodePic(merchant.getMerchantId()+"/"+0);
                            merchant.updateById();
                        }
                        return result;
                    }
...
//                    }
                ...
                if (conn != null){
                    try{
                        ...
                        LogFactory.error(this,"关闭mysql连接时发生异常!",e);
                    }
                }
            }
            return result;
...
//                    OutputStream os = null;
                    ...
                    if (!exists){
                        ...
                        Blob         blob = conn.createBlob();
                        blob.setBytes(1, bytes);
                        pics.setBusinessLicensePic(blob);
                        return picsService.addMerchantPics(pics);
                    }else{
                        Blob blob = picsService.getBusinessLicensePic(merchant.getMerchantId());
                        if(blob==null){
                            blob = conn.createBlob();
                        }else{
                            blob.setBytes(1,new byte[(int)blob.length()]);
                        ...
                        blob.setBytes(1,bytes);
                        result= picsService.updateBusinessLicensePic(merchant.getMerchantId(),blob);
                        if(result){
                            merchant.setBusinessLicensePic(merchant.getMerchantId()+"/"+1);
                            merchant.updateById();
                        }
                        return result;
                    }
...
//                    OutputStream os = null;
                    ...
                    if (!exists){
                        ...
                        Blob         blob = conn.createBlob();
                        blob.setBytes(1, bytes);
                        pics.setOtherPic(blob);
                        return picsService.addMerchantPics(pics);
                    }else{
                        Blob blob = picsService.getotherPic(merchant.getMerchantId());
                        if(blob==null){
                            blob = conn.createBlob();
                        }else{
                            blob.setBytes(1,new byte[(int)blob.length()]);
                        ...
                        blob.setBytes(1,bytes);
                        result = picsService.updateotherPic(merchant.getMerchantId(),blob);
                        if(result){
                            merchant.setOtherPic(merchant.getMerchantId()+"/"+2);
                            merchant.updateById();
                        }
                        return result;
                    }
