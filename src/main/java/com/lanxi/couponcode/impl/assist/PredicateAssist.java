    ...
    BiFunction<Commodity, OperateType, RetMessage>  checkCommodity = (c, o) -> {
        ...
        switch(o){
            case createCouponCode:
                ...
                return null;
            //上架 请求上架 要求 商品为 下架 或请求上架状态
            case shelveCommodity:
            case requestShelveCommodity:
                if(! CommodityStatus.shelvedWait.equals(c.getStatus())&&! CommodityStatus.unshelved.equals(c.getStatus()))
                    return new RetMessage(RetCodeEnum.fail, "非上架状态,无法操作!", null);
                return null;

            //修改,请求修改器,注销,删除 要求 商品为 下架状态
            case requestModifyCommodity:
            case modifyCommodity:
            case cancelCommodity:
            case deleteCommodity:
                ...
                return null;
            //下架 请求下架 要求 商品为 上架|请求下架状态
            case unshelveCommodity:
            case requestUnshelveCommodity:
                if(! CommodityStatus.unshelvedWait.equals(c.getStatus()) && ! CommodityStatus.shelved.equals(c.getStatus()))
                    return new RetMessage(RetCodeEnum.fail, "非下架状态,无法操作!", null);
                return null;
    ...
    BiFunction<Merchant, OperateType, RetMessage>   checkMerchant  = (m, o) -> {
        ...
        switch(o){
            ...
            case queryRequest:
                return null;
            case unfreezeMerchant:
                if(!MerchantStatus.freeze.equals(m.getMerchantStatus()))
                    return new RetMessage(RetCodeEnum.fail,"商户未冻结!",null);
                else
                    return null;
            default:
