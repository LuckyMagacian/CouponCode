	...
	public RetMessage<String> addOrder(String Phone, CommodityType Type, Long SkuCode, Integer Count, String Remark,
		...
		try {
			...
			if (CommodityStatus.shelved.equals(commodity.getStatus())) {
				// 判断交易序号是否重复
				if (orderService.isRepetition(MsgID, WorkDate, SRC)) {
					return new RetMessage<>(RetCodeEnum.exception, "交易序号重复", null);
				} else {
					...
					if (b) {
						...
						if (Count > 1) {
							...
							for (int i = 0; i < Count; i++) {
								...
								RetMessage<String> retMessage2 = couponService.generateCode(commodity.getMerchantId(),
								...
								if (RetCodeEnum.success.equals(retMessage2.getRetCode())) {
									...
									for (int n = 0; n < 8; n++) {
										retMessage3 = couponService.generateCode(commodity.getMerchantId(), SkuCode,
							...
							if (RetCodeEnum.success.equals(retMessage2.getRetCode())) {
								...
								RetMessage<String> retMessage3 = null;
								for (int i = 0; i < 8; i++) {
									retMessage3 = couponService.generateCode(commodity.getMerchantId(), SkuCode, "客户购买",
											Channel.buy.getChannel());
				...
				return new RetMessage<>(RetCodeEnum.fail,"此商品不是上架状态",null);
			}

		} catch (Exception e) {
			LogFactory.error(this, "生成订单时发生异常", e);
			return new RetMessage<>(RetCodeEnum.error, "生成订单时发生异常", null);
		}
		return retMessage;
	}
