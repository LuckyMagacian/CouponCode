...
public class AccountController implements com.lanxi.couponcode.spi.service.AccountService {
    ...
    public RetMessage<Boolean> freezeAccount(Long accountId, Long operaterId) {
        ...
        try {
            RetMessage message = null;
            OperateType ot = null;
            if (accountId.equals(operaterId)) {
    ...
    @Override
    public RetMessage<String> queryAccounts(String shopName, String phone, String merchantName, AccountType type,
        ...
        try {
            ...
            if (type != null) {
				if (AccountType.shopManager.equals(type.getValue())||AccountType.shopEmployee.equals(type.getValue())||AccountType.cancellation.equals(type.getValue())) 
					return new RetMessage<>(RetCodeEnum.fail,"管理员不能查询此类账户",null);
				else
				wrapper.eq("account_type", type+"");
			}else {
				wrapper.in("account_type", AccountType.admin+","+AccountType.merchantManager);
            }
            if (status != null) {
                wrapper.eq("status", status + "");
